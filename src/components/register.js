...
const Register = (props) => {
  ...
  const isValidInput = () => {
    const { userName, mobileNumber, address, password, confirmPassword, email } = registerDetails
    if (!commonHelpers.checkEmptyInput(userName)) {
      ...
      return false
    } else if (!commonHelpers.checkEmptyInput(email)) {
      commonHelpers.showMsg(translate('emailReq'), 'error')
      return false
    } else if (!commonHelpers.checkIsEmail(email)) {
      commonHelpers.showMsg(translate('emailErr'), 'error')
      return false
            ...
            <div className='p-md-4 p-3'>
              <Form className='register-form'>
                <FormGroup className='mb-4'>
                  ...
                  <FormControl className='mt-1 input-text text-capitalize' id='userName' name='userName' type='text' value={registerDetails.userName} onChange={(e) => handleChange(e)} maxLength={20} autoComplete='false' />
                </FormGroup>
                <FormGroup className='mb-4'>
                ...
                <FormGroup className='mb-4'>
                  <FormLabel className='input-label' htmlFor='email'>{translate('emailLabel', lang)}</FormLabel>
                  <FormControl className='mt-1 input-text' id='email' name='email' type='text' value={registerDetails.email} onChange={(e) => handleChange(e)} autoComplete='false' />
                </FormGroup>
                <FormGroup className='mb-4'>
